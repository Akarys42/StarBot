...
def add_node(base: Optional[ClassDef], name: str, node: dict) -> ClassDef:
    ...
    for key, value in node.items():
        parts = value.split(":")
        annotation = Name(id=SPECIAL_TYPES.get(parts[-1], value))

        if parts[0] == "optional":
            annotation = Subscript(value=Name(id="Optional"), slice=annotation)

        annotations.append(AnnAssign(target=Name(id=key), annotation=annotation, simple=1))
